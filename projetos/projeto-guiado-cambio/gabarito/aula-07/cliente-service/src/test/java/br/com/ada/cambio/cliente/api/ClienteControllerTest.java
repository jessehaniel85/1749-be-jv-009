...
class ClienteControllerTest {
    ...
    // O controller depende de DUAS interfaces pequenas (ISP); o teste mocka as duas.
    @MockitoBean
    ...
    void cadastraCliente() throws Exception {
        when(cadastroCliente.cadastrar(any())).thenReturn(clienteExemplo());
    ...
    void recusaCpfDuplicado() throws Exception {
        when(cadastroCliente.cadastrar(any())).thenThrow(new CpfDuplicadoException("43488428095"));
        ClienteRequest requisicao = new ClienteRequest("Ana Souza", "43488428095",
                LocalDate.of(1990, 5, 10), EstadoCivil.SOLTEIRO, Sexo.FEMININO);
    ...
    void buscaCliente() throws Exception {
        when(consultaCliente.buscarPorCpf("43488428095")).thenReturn(clienteExemplo());
    ...
    void clienteInexistente() throws Exception {
        when(consultaCliente.buscarPorCpf("00000000000"))
