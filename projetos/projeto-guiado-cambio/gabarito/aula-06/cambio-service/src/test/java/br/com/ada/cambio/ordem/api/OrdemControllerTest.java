...
/**
 * A fatia web continua leve mesmo com Feign no classpath: {@code @EnableFeignClients}
 * mora numa {@code @Configuration} que o {@code @WebMvcTest} nao varre.
 */
...
class OrdemControllerTest {
    ...
    @Test
    @DisplayName("404 do cliente-service (FeignException.NotFound) devolve 404")
    void clienteInexistente() throws Exception {
        when(ordemService.registrar(anyString(), anyString(), any(), anyString()))
                .thenThrow(FeignErros.naoEncontrado("/clientes/43488428095"));
    ...
    @Test
    @DisplayName("422 do cotacao-service devolve 422")
    void moedaRecusadaPeloVizinho() throws Exception {
        when(ordemService.registrar(anyString(), anyString(), any(), anyString()))
                .thenThrow(FeignErros.naoProcessavel("/cotacoes/EUR"));
    ...
    @Test
    @DisplayName("vizinho sem atender (RetryableException) devolve 503")
    ...
    @Test
    @DisplayName("nenhuma instancia no Eureka devolve 503 com mensagem propria")
    void semInstanciaRegistrada() throws Exception {
        when(ordemService.registrar(anyString(), anyString(), any(), anyString()))
        ...
        mockMvc.perform(post("/ordens").contentType(MediaType.APPLICATION_JSON).content(corpoValido()))
                ...
                .andExpect(jsonPath("$.mensagem").value(
                        org.hamcrest.Matchers.containsString("Eureka")));
    }
