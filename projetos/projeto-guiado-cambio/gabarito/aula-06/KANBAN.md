# Kanban — API de Câmbio (estado ao fim da **Aula 6**)
...
## Débito técnico registrado nesta aula

- `TratadorDeErros` importa `feign.*`: infraestrutura vazando até a borda. → alvo da **US-09**.
- `OrdemService` (pacote `dominio`) importa `infra`: a seta de dependência está invertida. → alvo da **US-09**.
